...
router.get("/:idOrSlug", async (req, res) => {
  try {
    ...
    let query = isId
      ? "SELECT * FROM api_news WHERE id = ?"
      : "SELECT * FROM api_news WHERE slug = ?";
...
router.post("/", async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute("SELECT * FROM api_news WHERE id = ?", [
      result.insertId,
    ]);
...
router.delete("/:id", async (req, res) => {
  try {
    const [result] = await pool.execute("DELETE FROM api_news WHERE id = ?", [
      req.params.id,
    ]);
