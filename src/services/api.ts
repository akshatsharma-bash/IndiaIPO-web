...
const getApiBaseUrl = () => {
  ...
  const defaultUrl = isDev ? "http://localhost:5000" : "https://www.indiaipo.in";
...
export const authApi = {
  ...
  getProfile: () =>
    fetch(`${API_BASE_URL}/auth/me`, { headers: headers() }).then(handleResponse),
  updateProfile: (data: Record<string, string>) =>
    fetch(`${API_BASE_URL}/auth/profile`, { method: "PUT", headers: headers(), body: JSON.stringify(data) }).then(handleResponse),
};
