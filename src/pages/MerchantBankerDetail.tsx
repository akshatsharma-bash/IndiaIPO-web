...
import {
  ...
  Home, Star, Zap, GitCompare, Check,
} from "lucide-react";
...
const ConnectModal = ({ banker, onClose }: { banker: Banker; onClose: () => void }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }}
      ...
      <div className="p-8 text-white relative text-center flex flex-col items-center" style={{ background: `linear-gradient(135deg, ${N}, #003380)` }}>
        ...
        <p className="font-extrabold text-lg uppercase tracking-wide" style={{ color: G }}>
          Save upto 20% on IPO Listing Expenses.
        </p>
      </div>
      <div className="p-8">
        <div className="pt-2">
          <Link to={`/merchant-contact?ipo_type=${banker.mcat_id === "list-of-sme-merchant-bankers" ? "SME IPO" : "Mainboard IPO"}&banker=${encodeURIComponent(banker.title)}`} className="block">
...
const MerchantBankerDetail = () => {
  ...
  useEffect(() => {
    const fetchBanker = async () => {
      ...
      try {
        // Try SME first
        let res = await fetch(`/api/bankers/slug/${slug}`);
        if (!res.ok) {
          // Try Mainboard
          res = await fetch(`/api/mainboard-bankers/slug/${slug}`);
        }
        ...
        if (res.ok) {
          ...
          try {
            ...
            if (Array.isArray(relatedIds) && relatedIds.length > 0) {
              const relRes = await fetch(`/api/bankers?ids=${relatedIds.join(',')}`);
              if (relRes.ok) {
        ...
        console.error("Error fetching banker:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <p className="mt-4 font-black text-slate-400 uppercase tracking-widest text-sm">Loading Profile...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <h1 className="text-3xl font-black text-[#001529] mb-2">Merchant Banker Not Found</h1>
          <p className="text-slate-500 mb-8 text-center max-w-md">We couldn't find the merchant banker profile you're looking for.</p>
          <Button asChild className="rounded-xl h-12 px-8">
            <Link to="/merchant-bankers/list-of-sme-merchant-bankers">Back to Directory</Link>
          </Button>
        </main>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#F8FAFC" }}>
      ...
      <SEOHead
        title={`${banker.meta_title || banker.title} | India IPO Merchant Banker`}
        description={banker.meta_desc || banker.description?.replace(/<[^>]*>?/gm, "").substring(0, 160)}
        keywords={banker.meta_keywords || "Merchant Banker, SME IPO, BRLM India"}
      />
      <Header />
      <main className="flex-1">
        <div className="pt-6 md:pt-12 pb-28 px-4 relative overflow-hidden"
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="flex flex-wrap items-center gap-2 mb-8 text-xs text-white/60 font-bold">
              ...
              <Link to="/merchant-bankers/list-of-sme-merchant-bankers" className="hover:text-white transition-colors">Merchant Bankers</Link>
              <ChevronRight className="h-3.5 w-3.5 text-white/20" />
              <Link to={banker.mcat_id === 'list-of-mainboard-merchant-bankers' ? "/merchant-bankers/list-of-mainboard-merchant-bankers" : "/merchant-bankers/list-of-sme-merchant-bankers"} className="hover:text-white transition-colors">
                {banker.mcat_id === 'list-of-mainboard-merchant-bankers' ? "Mainboard" : "SME"}
              </Link>
              <ChevronRight className="h-3.5 w-3.5 text-white/20" />
              <span className="text-white truncate max-w-[200px] sm:max-w-[400px]">{banker.title}</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-start gap-8">
              <div className="w-full max-w-[320px] aspect-square md:w-44 md:h-44 rounded-2xl bg-white flex items-center justify-center p-3 shadow-2xl shrink-0 border-4 border-white/10 overflow-hidden">
                {banker.image || banker.logo_url ? (
                  <img src={imgSrc} alt={banker.title} className="w-full h-full object-contain p-2"
                    onError={(e: any) => { e.target.style.display = "none"; }} />
                ) : (
                  <span className="text-5xl font-black" style={{ color: N }}>{banker.title?.[0]}</span>
                )}
              </div>
        ...
        <div className="container mx-auto px-4 -mt-14 relative z-20 pb-20">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 mb-8 overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              {[
                { label: "Total IPOs", value: banker.noOfiposofar || banker.total_ipos || "—" },
                { label: "Total Fund Raised(CR)", value: formatIndianNumber(banker.totalfundraised || banker.total_raised) || "—" },
                { label: "Avg IPO Size(CR)", value: formatIndianNumber(banker.avgiposize || banker.avg_size) || "—" },
                { label: "Avg Subscription", value: banker.avgsubscription ? banker.avgsubscription + "x" : (banker.avg_subscription ? banker.avg_subscription + "x" : "—") },
              ].map((s, i) => (
                <div key={i} className="flex flex-col items-center text-center py-6 px-4">
                  <p className="text-3xl md:text-4xl font-black mb-1" style={{ color: i % 2 === 0 ? N : G2 }}>{s.value}</p>
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-xl p-6 text-center" style={{ background: "rgba(0,21,41,0.05)", border: "1px solid rgba(0,21,41,0.1)" }}>
                        <p className="text-4xl font-black mb-1" style={{ color: N }}>{banker.nseemer || "—"}</p>
                        <p className="text-sm font-black text-slate-400 uppercase tracking-widest mt-1">NSE Emerge</p>
                      </div>
                      <div className="rounded-xl p-6 text-center" style={{ background: "rgba(245,158,8,0.08)", border: "1px solid rgba(245,158,8,0.2)" }}>
                        <p className="text-4xl font-black mb-1" style={{ color: G2 }}>{banker.bsesme || "—"}</p>
                        <p className="text-sm font-black text-slate-400 uppercase tracking-widest mt-1">BSE SME</p>
                      </div>
                    </div>
                    {banker.avglisting_gain && (
                      <div className="mt-4 p-4 rounded-xl text-center" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
                        <p className="text-2xl font-black" style={{ color: "#16a34a" }}>{banker.avglisting_gain}</p>
                        <p className="text-sm font-black text-slate-400 uppercase tracking-widest mt-1">Avg Listing Gain</p>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr style={{ background: "#F8FAFC" }}>
                          <th className="px-5 py-4 text-left text-sm font-black uppercase tracking-widest text-slate-500">Year</th>
                          <th className="px-5 py-4 text-center text-sm font-black uppercase tracking-widest text-slate-500">IPOs</th>
                          {yearwise[0]?.amount !== undefined && <th className="px-5 py-4 text-right text-sm font-black uppercase tracking-widest text-slate-500">Amount</th>}
                        </tr>
                      </thead>
                        ...
                        {yearwise.map((item: any, idx: number) => (
                          <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"}>
                            ...
                            <td className="px-5 py-4 text-center">
                              <span className="inline-flex items-center justify-center px-4 py-1.5 rounded-full text-sm font-black"
                                style={{ background: "rgba(245,158,8,0.12)", color: G2 }}>
                                {item.count || item.value || item.ipos || item.no_of_ipos || 0} IPOs
                              </span>
                            </td>
                            {item.amount !== undefined && (
                              <td className="px-5 py-4 text-right font-bold text-base text-slate-600">{item.amount}</td>
                            )}
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr style={{ background: "#F8FAFC" }}>
                          <th className="px-5 py-4 text-left text-sm font-black uppercase tracking-widest text-slate-500">Category</th>
                          <th className="px-5 py-4 text-right text-sm font-black uppercase tracking-widest text-slate-500">Count / Rate</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                          ...
                          return (
                            <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"}>
                              ...
                              <td className="px-5 py-4 text-right">
                                <span className="font-black text-base" style={{ color: colors[idx % colors.length] }}>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr style={{ background: "#F8FAFC" }}>
                          <th className="px-5 py-4 text-left text-sm font-black uppercase tracking-widest text-slate-500">Size Range</th>
                          <th className="px-5 py-4 text-right text-sm font-black uppercase tracking-widest text-slate-500">No. of IPOs</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                      ...
                      >
                        <div className="w-12 h-12 rounded-lg bg-white border border-slate-200 p-2 shrink-0 flex items-center justify-center overflow-hidden">
                          {rb.logo_url || rb.image ? (
                            <img src={getImageUrl(rb.logo_url || rb.image)} alt={rb.title || rb.name} className="w-full h-full object-contain" />
                          ) : (
                            <Building2 className="w-6 h-6 text-slate-200" />
                          )}
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold text-base text-[#001529] group-hover:text-primary transition-colors truncate">{rb.title || rb.name}</p>
                          <p className="text-sm font-black text-slate-400 uppercase tracking-widest mt-0.5">View Profile <ChevronRight className="inline-block w-4 h-4" /></p>
                        </div>
                      </Link>
                    ))}
                  </div>
            ...
            <div className="space-y-5">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-24">
                ...
                <div className="border-t border-slate-100 p-6">
                  <p className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Quick Summary</p>
                  <div className="space-y-3">
                    {[
                      { label: "Total IPOs", val: banker.noOfiposofar || banker.total_ipos },
                      { label: "Fund Raised", val: formatIndianNumber(banker.totalfundraised || banker.total_raised) },
                      { label: "Avg IPO Size", val: formatIndianNumber(banker.avgiposize || banker.avg_size) },
                      ...
                      <div key={i} className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-black" style={{ color: N }}>{s.val}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* <div className="rounded-2xl p-6 relative overflow-hidden" style={{ background: N }}>
                ...
                <p className="text-white/65 text-sm mb-6 leading-relaxed">Connect with top expert merchant bankers for a seamless IPO journey.</p>
                <Link to="/ipo-eligibility-check"
                  ...
                  style={{ background: `linear-gradient(135deg, ${G}, ${G2})`, color: N }}>
                  Check Your IPO Feasibility <ArrowRight className="h-4 w-4" />
                </Link>
              </div> */}
            </div>
          </div>
...
export default MerchantBankerDetail;
