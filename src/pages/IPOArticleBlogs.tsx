...
const IPOArticleBlogs = () => {
  ...
  const itemListSchema = blogs.length > 0 ? {
    ...
    "@type": "ItemList",
    "name": "IPO Article Blogs | India IPO — Expert Analysis",
    "description": "Expert IPO blogs and in-depth articles on India IPO. Comprehensive analysis, reviews and market research on IPO listings.",
    "url": `${siteUrl}/blogs`,
        ...
        <div className="container mx-auto px-4 -mt-10 relative z-20">
          ...
          {!loading && blogs.length > 0 && page === 1 && !debouncedSearch && (
            <motion.div
              ...
              className="mb-20"
            >
              <Link to={`/blogs/${blogs[0].slug}`} className="group relative block overflow-hidden rounded-[2.5rem] bg-slate-900 shadow-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-14">
                <AnimatePresence>
                  {blogs.slice(page === 1 && !debouncedSearch ? 1 : 0).map((blog, idx) => (
                    ...
                    >
                      <Link to={`/blogs/${blog.slug}`} className="group block space-y-6">
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-6 pt-4">
              <Button asChild className="h-16 px-12 rounded-2xl bg-amber-400 hover:bg-amber-300 text-slate-900 font-black text-sm uppercase tracking-widest shadow-2xl shadow-amber-400/20 transition-transform hover:scale-105">
                <Link to="/ipo-eligibility-check">Evaluate Eligibility <Zap className="ml-2 w-5 h-5" /></Link>
              </Button>
              <Button asChild variant="outline" className="h-16 px-12 rounded-2xl border-white bg-transparent text-white font-black text-sm uppercase tracking-widest hover:bg-transparent hover:text-white">
                <Link to="/contact">Speak with Analysts</Link>
              </Button>
            </div>
          </motion.div>
        </div>
...
export default IPOArticleBlogs;
