...
const Reports = () => {
  ...
  const effectiveSlug = slug || (pathname === "/mainline-ipos" ? "mainline-ipo-report" : pathname === "/sme-ipos" ? "sme-ipo-report" : pathname === "/sme-ipo-sector" ? "sme-ipos-by-sector" : pathname === "/mainboard-ipo-sector" ? "mainboard-ipos-by-sector" : null);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      else if (effectiveSlug === "mainline-ipo-report") params.category = "mainline";
  ...
  const getSubtitle = () => {
    ...
    switch (effectiveSlug) {
      case "upcoming-ipo-calendar": return "Track upcoming IPOs with real-time updates, timelines and key issue details to stay ahead of market opportunities.";
      case "mainline-ipo-report": return "Access real-time data and performance insights on mainboard IPOs, including pricing trends, subscription levels and listing outcomes.";
      case "sme-ipo-report": return "Track SME IPOs with real-time data, GMP trends, subscription demand and listing performance insights.";
      case "sme-ipos-by-sector": return "Track sector-wise SME IPO trends with real-time data on issue activity, pricing dynamics and listing outcomes.";
      case "mainboard-ipos-by-sector": return "Analyse sector-wise mainboard IPO activity with real-time data on issue trends, capital flows and listing performance.";
      default: return "Professional real-time tracking of IPO listings and market developments for informed investment decisions.";
    }
  };

  const getCards = () => {
    const commonCards = [
      { icon: PieChart, title: "Professional Analysis", desc: "Each IPO is tracked with verified data, covering GMP trends, pricing updates and market sentiment indicators.", gold: false },
      { icon: Calendar, title: "Real-Time Updates", desc: "Stay updated with daily changes in GMP, price bands, issue dates and listing schedules as announced.", gold: false },
      { icon: TrendingUp, title: "IPO Eligibility", desc: "Curious about your own company's IPO journey? Let our experts guide you through complex SEBI regulations.", gold: true },
    ];
    ...
    if (effectiveSlug === "upcoming-ipo-calendar") {
      return [
        { icon: PieChart, title: "Professional Analysis", desc: "Each IPO is tracked with verified data, covering GMP trends, pricing updates and market sentiment indicators.", gold: false },
        { icon: Calendar, title: "Real-Time Updates", desc: "Stay updated with daily changes in GMP, price bands, issue dates and listing schedules as announced.", gold: false },
        { icon: TrendingUp, title: "IPO Eligibility", desc: "Planning to take your company public? Consult our experts for a seamless listing.", gold: true },
      ];
    }
    if (effectiveSlug === "mainline-ipo-report") {
      return [
        { icon: PieChart, title: "Professional Analysis", desc: "Each IPO is tracked with verified data, covering GMP trends, pricing updates and market sentiment indicators.", gold: false },
        { icon: Calendar, title: "Real-Time Updates", desc: "Stay updated with daily changes in GMP, price bands, issue dates and listing schedules as announced.", gold: false },
        { icon: TrendingUp, title: "Mainboard Listing", desc: "Ready for the big stage? Let us help you navigate Mainboard IPO regulations.", gold: true },
      ];
    }
    if (effectiveSlug === "sme-ipo-report") {
      return [
        { icon: PieChart, title: "Growth Analytics", desc: "Specialized tracking of high-growth SME issues across BSE SME and NSE Emerge platforms.", gold: false },
        { icon: Calendar, title: "SME Fund Raising", desc: "Detailed insights into how small enterprises are successfully tapping into the primary markets.", gold: false },
        { icon: TrendingUp, title: "SME IPO Advisor", desc: "Need guidance for an SME IPO? Our experts specialize in small-cap listing compliance.", gold: true },
      ];
    }
    return commonCards;
  };
  ...
  const getTitle = () => {
    ...
    switch (effectiveSlug) {
      ...
      case "mainline-ipo-report": return "Mainline IPO Updates";
      case "sme-ipo-report": return "SME IPO Updates";
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="px-5 py-3 bg-slate-50/80 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase">TBA</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                              ...
                              <tr key={`desktop-${item.id}`}
                                ...
                                <td className="px-5 py-6">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <p className="font-black text-[#001529] text-[16px] group-hover:text-blue-700 transition-colors">{item.issuer_company}</p>
                                ...
                                <td className="px-5 py-6 text-center">
                                  <span className={`inline-block px-4 py-1.5 rounded-full text-[10px] font-black uppercase shadow-sm border ${statusColor[String(displayStatus)] || "bg-slate-100 text-slate-500 border-slate-200"
                                    }`}>
                                    {displayStatus}
                                  </span>
                                </td>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="px-4 pb-4 mt-auto">
                                {item.blog_slug ? (
                                  <Button asChild size="sm" className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-600/10 h-10 transition-all">
                                    <Link to={`/ipo-blogs/${item.blog_slug}`}>
              ...
              <div className="lg:w-80 shrink-0 space-y-6 order-1 lg:order-2 lg:sticky lg:top-[120px] lg:self-start lg:max-h-[calc(100vh-140px)] lg:overflow-y-auto lg:pb-6 scrollbar-hide">
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-5">
                  ...
                  <div>
                    ...
                    <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPagination(p => ({ ...p, page: 1 })); }}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Issue Closed (Unlisted)">Closed (Unlisted)</SelectItem>
                        <SelectItem value="Listing Today">Listing Today</SelectItem>
                  ...
                  <div>
                    ...
                    <Select value={pathname} onValueChange={(v) => navigate(v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="/mainline-ipos">Mainline IPO Updates</SelectItem>
                        <SelectItem value="/sme-ipos">SME IPO Updates</SelectItem>
                        <SelectItem value="/sme-ipo-sector">SME IPOs by Sector</SelectItem>
                        <SelectItem value="/mainboard-ipo-sector">Mainboard IPOs by Sector</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  {isSectorPage && (
                    <div>
                      ...
                      <Select value={sectorFilter || "all"} onValueChange={(v) => navigate(v === "all" ? pathname : `${pathname}?sector=${encodeURIComponent(v)}`)}>
                        <SelectTrigger className="h-11 rounded-xl bg-slate-50 border-slate-100 text-sm font-bold">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between group/stat">
                        ...
                        <span className="text-white font-black text-[15px] group-hover:text-[#f59e08] transition-colors">₹{sizeStats.highest.toFixed(2)}</span>
                      </div>
                      <div className="w-full h-px bg-white/5" />
                      <div className="flex items-center justify-between group/stat">
                        <span className="text-white/50 text-[13px] font-medium">Median</span>
                        <span className="text-white font-black text-[15px] group-hover:text-[#f59e08] transition-colors">₹{sizeStats.median.toFixed(2)}</span>
                      </div>
                      <div className="w-full h-px bg-white/5" />
                      <div className="flex items-center justify-between group/stat">
                        <span className="text-white/50 text-[13px] font-medium">Lowest</span>
                        <span className="text-white font-black text-[15px] group-hover:text-[#f59e08] transition-colors">₹{sizeStats.lowest.toFixed(2)}</span>
                      </div>
                    </div>
                    <p className="text-[10px] text-white/30 mt-6 italic">Summary of current listed records</p>
                  </div>
                </div>
                ...
                <Link to="/ipo-eligibility-check" className="block group">
