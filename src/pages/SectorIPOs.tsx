...
const SectorIPOs = () => {
  ...
  const visibleSectors = useMemo(() => {
    ...
    // in descending order of click/selection time (first in selectedSectors array is last clicked/top-most)
  ...
  const handleSectorToggle = (sectorName: string) => {
    ...
    if (isCurrentlySelected) {
      ...
      // Toggle on -> add to the FRONT of the selected list (so the most recently clicked shows first)
    ...
    if (updatedIds.length === 0) {
      ...
      navigate(isMainboard ? `/ipos/sector/${updatedIds.join(",")}` : `/sme-ipos/sector/${updatedIds.join(",")}`);
    }
  };
  ...
  const activeSectorNamesStr = selectedSectors.join(", ");
  const seoTitle = activeSectorNamesStr 
    ? `${activeSectorNamesStr} Sectors - ${title} | India IPO` 
    : `${title} by Sector | India IPO`;
  const seoDescription = activeSectorNamesStr
    ? `Explore the latest ${title} in the ${activeSectorNamesStr} sectors. Get detailed listing dates, issue sizes, GMP trends, and valuation metrics.`
    : `View ${title} categorized by industry sectors. Get detailed insights into company size and listing years.`;
                        ...
                        items.map((item, idx) => (
                          <tr key={item.id} className={cn(
                            ...
                            <td className="py-4 px-5">
                              {item.blog_slug ? (
                                <Link to={`/ipo-blogs/${item.blog_slug}`} className="text-[#3B71CA] font-bold text-[14px] hover:underline">
                                  {item.issuer_company}
                                </Link>
                              ) : (
                ...
                <div className="p-2 max-h-[40vh] overflow-y-auto custom-scrollbar">
                  {!sectorSearch && (
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold transition-colors mb-1",
