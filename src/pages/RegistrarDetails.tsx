  ...
  if (!registrar) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        ...
        <Button asChild><Link to="/ipo-registrar-list">Back to List</Link></Button>
      </div>
    );
  }
    ...
    return (
      <div className="mt-20 relative">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-black font-heading text-foreground mb-4 tracking-tight">
            {title}
          </h2>
          <div className="w-28 h-2 bg-primary mx-auto rounded-full"></div>
        </div>
        ...
        <div className="group/scroll-section relative">
          ...
          <button 
            onClick={() => scroll('left')}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-white/90 border border-border shadow-xl rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all opacity-0 group-hover/scroll-section:opacity-100 -left-6 hidden md:flex"
          >
          ...
          <button 
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-white/90 border border-border shadow-xl rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all opacity-0 group-hover/scroll-section:opacity-100 -right-6 hidden md:flex"
          >
          ...
          >
            {ipos.map((ipo) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-card border border-border rounded-[2rem] p-6 md:p-8 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all group/card overflow-hidden relative min-w-[300px] md:min-w-[340px] flex-shrink-0"
              >
                ...
                <div className="mb-6 mt-2">
                  <h3 className="text-lg md:text-xl font-black font-heading text-foreground group-hover/card:text-primary transition-colors leading-snug">
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-center justify-between py-3 border-b border-border/50">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Listing Date</span>
                    <span className="text-sm font-black text-foreground">{formatDate(ipo.listing_date)}</span>
                  </div>
                  <div className="flex items-center justify-between py-3 border-b border-border/50">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Lot Size</span>
                    <span className="text-sm font-black text-foreground">{ipo.lot_size || 'TBA'}</span>
                  </div>
                  <div className="flex items-center justify-between py-3">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Issue Size</span>
                    <span className="text-base font-black text-primary italic">₹{ipo.issue_size || '0'} Cr.</span>
                  </div>
                </div>

                <Button asChild className="w-full h-12 rounded-xl font-black bg-primary/10 hover:bg-primary text-primary hover:text-white transition-all border-none text-sm">
                  <Link to={ipo.blog_slug ? `/ipo-blogs/${ipo.blog_slug}` : `/ipo-registrar-list/${slug}`}>
                    View Details
                  </Link>
                </Button>
              </motion.div>
  ...
  const registrarSchemas: object[] = [
    {
      ...
      "url": `${siteUrl}/ipo-registrar-list/${registrar.slug}`,
      "image": registrar.image ? (registrar.image.startsWith('http') ? registrar.image : `${siteUrl}/${registrar.image}`) : `${siteUrl}/favicon.png`,
      "address": {
        "@type": "PostalAddress",
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(registrarSchemas) }}
      />
      <Header />
      ...
      <main className="pb-20">
        ...
        <div className="h-48 md:h-64 bg-primary relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 h-full flex items-end pb-8 relative z-10">
            <Link to="/ipo-registrar-list" className="mb-auto mt-8 flex items-center gap-2 text-white/80 hover:text-white transition-colors font-medium font-poppins">
        ...
        <div className="container mx-auto px-4 -mt-12 relative z-20">
          <div className="bg-card border border-border rounded-[2.5rem] p-6 md:p-10 shadow-2xl">
            <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
              <div className="w-full max-w-[280px] aspect-square md:w-40 md:h-40 rounded-3xl bg-white border border-border shadow-inner p-4 flex items-center justify-center shrink-0 overflow-hidden">
                <img
        ...
        {faqs.length > 0 && (
          <section className="mb-24">
            <div className="container mx-auto px-4 max-w-5xl">
              <h2 className="text-3xl md:text-4xl font-black font-heading text-foreground mb-8 flex items-center gap-4">
              ...
              <Accordion type="single" collapsible className="w-full space-y-4">
                {faqs.map((faq, idx) => (
                  <AccordionItem key={idx} value={`item-${idx}`} className="border border-border rounded-2xl overflow-hidden bg-white shadow-sm">
                    <AccordionTrigger className="px-6 py-5 hover:no-underline font-black text-foreground hover:bg-muted/30 transition-colors text-left">
                      {faq.question}
                    </AccordionTrigger>
                    <AccordionContent className="px-6 py-5 text-muted-foreground leading-relaxed pb-8">
                      {faq.answer}
                    </AccordionContent>
                  </AccordionItem>
                ))}
