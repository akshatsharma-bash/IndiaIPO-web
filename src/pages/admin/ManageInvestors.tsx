      ...
      <div className="space-y-6 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-6 rounded-2xl border border-primary/20 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => fetchEnquiries(1, search)} disabled={loading} className="rounded-xl h-9 gap-2 text-xs font-semibold">
        ...
        <div className="relative max-w-sm">
          ...
          <Input
            ...
            className="pl-10 h-10 rounded-xl bg-card border-border shadow-sm focus-visible:ring-primary text-sm"
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 border-2 border-dashed rounded-2xl bg-card border-border">
                ...
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    {["#", "Investor", "Contact", "Ticket Size", "Industry", "ROI / Tenure", "Inv. Type", "Query", "Actions"].map((col) => (
                      <th key={col} className="text-left py-3.5 px-4 text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground whitespace-nowrap">
                        {col}
                      </th>
                    ))}
                  </tr>
            ...
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4 bg-card p-4 rounded-xl border border-border">
                ...
                <div className="flex items-center gap-1.5">
                  <Button variant="outline" size="sm" className="h-8 px-4 rounded-xl text-xs font-bold" disabled={page <= 1} onClick={() => { window.scrollTo(0, 0); fetchEnquiries(page - 1); }}>← Prev</Button>
                  <div className="flex items-center gap-1">
                    {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
                      let pn = i + 1;
                      if (totalPages > 5 && page > 3) { pn = page - 3 + i; if (pn > totalPages) pn = totalPages - (4 - i); }
                      return (
                        <Button key={pn} variant={page === pn ? "default" : "ghost"} className="h-8 w-8 p-0 text-xs font-bold rounded-xl" onClick={() => { window.scrollTo(0, 0); fetchEnquiries(pn); }}>{pn}</Button>
                      );
                    })}
                  </div>
                  <Button variant="outline" size="sm" className="h-8 px-4 rounded-xl text-xs font-bold" disabled={page >= totalPages} onClick={() => { window.scrollTo(0, 0); fetchEnquiries(page + 1); }}>Next →</Button>
                </div>
              </div>
            )}
          </>
        ...
        <Dialog open={!!selectedEnquiry} onOpenChange={(open) => !open && setSelectedEnquiry(null)}>
          <DialogContent className="max-w-3xl bg-white border-primary/20 p-0 overflow-hidden rounded-2xl">
              ...
              <DialogHeader>
                <div className="flex items-center gap-4">
                  ...
                  {selectedEnquiry?.ticket_size && (
                    <Badge className="ml-auto bg-emerald-100 text-emerald-800 border-none px-3 py-1 font-bold text-[11px] rounded-xl">
                      {selectedEnquiry.ticket_size}
                    </Badge>
                  )}
                </div>
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/20 flex justify-end gap-2">
              <Button variant="ghost" size="sm" className="text-xs font-bold gap-1.5 rounded-xl" onClick={() => selectedEnquiry && copyToClipboard(selectedEnquiry)}>
