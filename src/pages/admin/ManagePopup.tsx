...
const ManagePopup = () => {
  ...
  const toggleActive = async (checked: boolean) => {
    ...
    try {
      ...
      toast.success(`Popup ${checked ? "activated" : "deactivated"}!`);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    if (form.title.length > 30) {
      toast.error(`Title cannot exceed 30 characters (current: ${form.title.length})`);
      return;
    }


    if (form.description.length > 300) {
      toast.error(`Description cannot exceed 300 characters (current: ${form.description.length})`);
      return;
    }
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  placeholder="e.g. New Release (only 30 charater allowed)"
                  required
                  maxLength={30}
                  value={form.title}
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-[10px] ${form.description.length > 200 ? "text-destructive font-bold" : "text-muted-foreground"}`}>
