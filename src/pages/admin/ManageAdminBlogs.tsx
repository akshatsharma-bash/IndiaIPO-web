...
import { Plus, Pencil, Trash2, Loader2, Image as ImageIcon, ChevronLeft, ChevronRight, Layout, BarChart, Info, List, Link as LinkIcon, Database, HelpCircle, Bold, Italic, Type, Palette, RemoveFormatting, Code, Heading1, Heading2, Heading3, Download, FileText, Search, TrendingUp, Activity, Globe } from "lucide-react";
...
const ManageAdminBlogs = () => {
    ...
    if (s.startsWith('[') && s.endsWith(']')) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          // Improved: Filter out items that are generic labels (common in scrapped data)
          ...
          if (filtered.length > 0) {
            // Prefer the item with digits if multiple items remain
  ...
  const handleSave = async () => {
    ...
    // Proactively populate timelines from Additional Details if they are empty
    ...
    try {
      ...
      const cleanDateOnly = (v: any) => {
        const s = String(v || '').trim();
        // Updated regex to support both date and time
      ...
      // Promoter holding - ensure they are saved as single-item arrays for consistency and proper display
      ...
      // Ensure both slug fields are present and identical for backend consistency
      ...
      // linked_digest_id removal logic removed
  ...
  const handleEdit = async (b: AdminBlog) => {
    try {
      ...
      if (res.ok) {
        ...
        // THE SMART DISTRIBUTION LOGIC: 
        // Handles cases where scrapper might have pushed both values into one column
        // or swapped labels with values.
        ...
        if (preNums.length === 0 && postNums.length >= 2) {
          // If Pre is empty but Post has 2+ values, distribute first two to Pre and Post
          ...
          // Single value in Post, likely belongs to Pre
          cleanedData.promotor_hold_pre_issue = postNums[0];
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string, folder = "admin_blogs", isPdf = false) => {
    ...
    try {
      ...
      console.error(err);
      const errMsg = `Failed to upload ${fileTypeLabel}`;
      if (file.size > 25 * 1024 * 1024) {
        toast.error(`cannot upload ${fileTypeLabel} >25 mb`, { id: tId });
      } else {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) { setForm(emptyForm); setEditingId(null); } }}>
            ...
            <DialogContent
              className="max-w-4xl max-h-[90vh] overflow-y-auto w-full"
              onInteractOutside={(e) => {
                e.preventDefault();
              }}
              onEscapeKeyDown={(e) => {
                ...
                <Tabs value={modalTab} onValueChange={setModalTab} defaultValue="basic" className="w-full">
                  {form.category === 'daily_reporter' ? (
                    <TabsList className="grid grid-cols-2 md:grid-cols-3 mb-6">
                      <TabsTrigger value="basic" className="flex items-center gap-1.5"><Info className="w-3.5 h-3.5" /> Basic Info</TabsTrigger>
                      <TabsTrigger value="content" className="flex items-center gap-1.5"><Layout className="w-3.5 h-3.5" /> Content & FAQs</TabsTrigger>
                      <TabsTrigger value="seo" className="flex items-center gap-1.5"><LinkIcon className="w-3.5 h-3.5" /> SEO</TabsTrigger>
                    </TabsList>
                        ...
                        {form.image && (
                          <div className="mt-3 relative inline-block">
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1.5 font-medium flex items-center gap-1">
                              <ImageIcon className="w-3 h-3" /> Preview of uploaded image
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    {form.category === 'daily_reporter' && (
                      <div className="mt-4 p-4 bg-blue-50/50 border border-blue-100 rounded-xl">
                        ...
                        <p className="text-[10px] text-blue-600/70 mt-1.5 font-medium italic">
                          * Enter the news heading here. This will show up with a button linking to the News page.
                        </p>
                      </div>
                    )}
                  </TabsContent>
                    ...
                    <>
                      <TabsContent value="gmp" className="space-y-6">
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <h4 className="text-sm font-semibold">GMP Update History (Daily Updates)</h4>
                            <Button type="button" variant="outline" size="sm" onClick={() => setGmpRows([...gmpRows, { date: '', price: gmpRows[gmpRows.length - 1]?.price || '', gmp: '', updated: '' }])}>
                          ...
                          <div className="overflow-x-auto border rounded-lg">
                            <table className="w-full text-xs">
                              <thead className="bg-muted/50 border-b">
                                <tr>
                                  <th className="p-2 text-left font-semibold">GMP Date</th>
                                  <th className="p-2 text-left font-semibold">IPO Price Band</th>
                                  <th className="p-2 text-left font-semibold">Latest GMP</th>
                                  <th className="p-2 text-left font-semibold">GMP Last Updated</th>
                              ...
                              <tbody>
                                  ...
                                  <tr key={idx} className="border-b last:border-0 hover:bg-muted/30">
                                    <td className="p-2">
                                      <div className="flex gap-1">
                                        <Input className="h-8 text-xs flex-1" value={row.date} onChange={(e) => { const n = [...gmpRows]; n[idx].date = e.target.value; setGmpRows(n); }} placeholder="dd MMM, yyyy" />
                                        <Popover>
                                    ...
                                    <td className="p-2">
                                      <Input className="h-8 text-xs" value={row.price} onChange={(e) => { const n = [...gmpRows]; n[idx].price = e.target.value; setGmpRows(n); }} placeholder="Enter Price" />
                                    </td>
                                    <td className="p-2">
                                      <Input className="h-8 text-xs" value={row.gmp} onChange={(e) => { const n = [...gmpRows]; n[idx].gmp = e.target.value; setGmpRows(n); }} placeholder="Enter GMP" />
                                    </td>
                                    <td className="p-2">
                                      <div className="flex gap-1">
                                        <Input className="h-8 text-xs flex-1" value={row.updated} onChange={(e) => { const n = [...gmpRows]; n[idx].updated = e.target.value; setGmpRows(n); }} placeholder="dd MMM, yyyy | hh:mm a" />
                                        <Button
                                          variant="ghost"
                                ...
                                {gmpRows.length === 0 && (
                                  <tr><td colSpan={5} className="p-4 text-center text-muted-foreground italic">No GMP data added. Click "Add GMP Update" to start.</td></tr>
                                )}
                              </tbody>
                            </table>
                          </div>
                      ...
                      <TabsContent value="financials" className="space-y-6">
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Button type="button" variant="outline" size="sm" onClick={() => setFinancialRows([{ ended: '', assets: '', revenue: '', profit: '', networth: '', reserves: '', borrowing: '' }, ...financialRows])}>
                              ...
                              <tbody>
                                {financialRows.map((row, idx) => (
                                  <tr key={idx} className="border-b last:border-0 hover:bg-muted/30">
                                    ...
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.assets} onChange={(e) => { const n = [...financialRows]; n[idx].assets = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].assets = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="56.67" /></td>
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.revenue} onChange={(e) => { const n = [...financialRows]; n[idx].revenue = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].revenue = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="45.63" /></td>
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.profit} onChange={(e) => { const n = [...financialRows]; n[idx].profit = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].profit = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="4.11" /></td>
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.networth} onChange={(e) => { const n = [...financialRows]; n[idx].networth = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].networth = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="22.01" /></td>
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.reserves} onChange={(e) => { const n = [...financialRows]; n[idx].reserves = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].reserves = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="10.50" /></td>
                                    <td className="p-1"><Input className="h-8 text-[11px] px-2" value={row.borrowing} onChange={(e) => { const n = [...financialRows]; n[idx].borrowing = e.target.value.replace(/[^0-9.]/g, ""); setFinancialRows(n); }} onBlur={(e) => { const n = [...financialRows]; n[idx].borrowing = formatIndianNumber(e.target.value); setFinancialRows(n); }} placeholder="17.17" /></td>
                        ...
                        <div className="pt-4 border-t">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="text-sm font-semibold">Competitive Strengths</h4>
                      ...
                      <div className="space-y-8">
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button type="button" variant="outline" size="sm" className="h-8 text-[10px] font-bold uppercase tracking-wider" onClick={() => setRecentIpoRows([...recentIpoRows, ''])}>
                          ...
                          <div className="space-y-2">
                            {recentIpoRows.map((s, idx) => (
                              <div key={idx} className="flex gap-2 group animate-in fade-in slide-in-from-left-2 duration-200">
                                <Input className="h-9 text-sm focus:ring-blue-500" value={s} onChange={(e) => { const n = [...recentIpoRows]; n[idx] = e.target.value; setRecentIpoRows(n); }} placeholder="e.g. Company X is in advanced talks for ₹2,000 Cr IPO" />
                            ...
                            {recentIpoRows.length === 0 && (
                              <div className="text-[11px] text-muted-foreground italic p-4 border border-dashed rounded-lg text-center">No points added. Click "Add Point" to start.</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button type="button" variant="outline" size="sm" className="h-8 text-[10px] font-bold uppercase tracking-wider" onClick={() => setPeFundingRows([...peFundingRows, ''])}>
                              <Plus className="w-3 h-3 mr-1" /> Add Point
                            </Button>
                          </div>
                          <div className="space-y-2">
                            {peFundingRows.map((s, idx) => (
                              <div key={idx} className="flex gap-2 group animate-in fade-in slide-in-from-left-2 duration-200">
                                <Input className="h-9 text-sm focus:ring-emerald-500" value={s} onChange={(e) => { const n = [...peFundingRows]; n[idx] = e.target.value; setPeFundingRows(n); }} placeholder="e.g. Startup Y raised $50M in Series B round" />
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button type="button" variant="outline" size="sm" className="h-8 text-[10px] font-bold uppercase tracking-wider" onClick={() => setBusinessEconomicRows([...businessEconomicRows, ''])}>
                              <Plus className="w-3 h-3 mr-1" /> Add Point
                            </Button>
                          </div>
                          <div className="space-y-2">
                            {businessEconomicRows.map((s, idx) => (
                              <div key={idx} className="flex gap-2 group animate-in fade-in slide-in-from-left-2 duration-200">
                                <Input className="h-9 text-sm focus:ring-amber-500" value={s} onChange={(e) => { const n = [...businessEconomicRows]; n[idx] = e.target.value; setBusinessEconomicRows(n); }} placeholder="e.g. GDP growth projected at 7.2% for FY25" />
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button type="button" variant="outline" size="sm" className="h-8 text-[10px] font-bold uppercase tracking-wider" onClick={() => setGeopoliticalRows([...geopoliticalRows, ''])}>
                              <Plus className="w-3 h-3 mr-1" /> Add Point
                            </Button>
                          </div>
                          <div className="space-y-2">
                            {geopoliticalRows.map((s, idx) => (
                              <div key={idx} className="flex gap-2 group animate-in fade-in slide-in-from-left-2 duration-200">
                                <Input className="h-9 text-sm focus:ring-indigo-500" value={s} onChange={(e) => { const n = [...geopoliticalRows]; n[idx] = e.target.value; setGeopoliticalRows(n); }} placeholder="e.g. New trade agreement signed between India and UAE" />
                  ...
                  <TabsContent value="seo" className="space-y-6">
                    ...
                    {form.category === 'ipo_updates' && (
                      <div className="space-y-4 pt-4 border-t">
                        ...
                        {/* DRHP Status Selector */}
                        <div className="flex items-center gap-4">
                          <div className="w-56">
                            <label className="text-xs font-medium mb-1 block">Select DRHP Status</label>
                            <Select
                        ...
                        {/* RHP + DRHP — only when NOT confidential */}
                        {form.confidential !== '1' && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                        ...
                        {/* Confidential DRHP multi-row section — only when confidential = '1' */}
                        {form.confidential === '1' && (
                                    ...
                                    <div className="flex gap-2 items-center">
                                      ...
                                      <div className="relative shrink-0">
                                        <input
                                          ...
                                          className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                        />
                                        <Button type="button" variant="outline" className="h-8 text-xs pointer-events-none">
                                          <Upload className="h-3 w-3 mr-1" /> Upload
                                        </Button>
                                      </div>
                                    </div>
                                    {row.pdf && (
                                      <p className="text-[10px] text-amber-700 mt-1 flex items-center gap-1 truncate">
                                        <FileText className="w-3 h-3 shrink-0" />
                                        <span className="truncate">{row.pdf}</span>
                                      </p>
                                    )}
                                  </div>
                                </div>
                              ))}
                              {confidentialDrhpRows.length === 0 && (
                                <p className="text-[11px] text-amber-600 italic text-center py-3">No entries. Click "Add Entry" to start.</p>
                              )}
                            </div>
                          </div>
                        )}
                  ...
                  {form.category === 'daily_reporter' ? null : null}
                </Tabs>
