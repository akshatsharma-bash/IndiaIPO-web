...
const ManageLeads = () => {
  ...
  useEffect(() => {
    fetchLeads(1);
    const interval = setInterval(() => fetchLeads(page), 30000);
  ...
  const handleExport = async () => {
    try {
      ...
      const csv = "Name,Email,Phone,Company,Message,Date,Read\n" +
        allLeads.map((l: any) => `"${l.name}","${l.email}","${l.mobile || ""}","${l.Cname || ""}","${l.message.replace(/"/g, '""')}","${new Date(l.created_at).toLocaleDateString()}","${l.is_read}"`).join("\n");
    ...
    <AdminLayout>
      <div className="space-y-6">
                        ...
                        return pages.map((p, i) => (
                          p === '...' ? (
                            ...
                            <Button
                              ...
                              className={`h-8 w-8 p-0 text-[10px] rounded-md transition-all ${page === p ? "bg-accent text-accent-foreground shadow-sm" : ""}`}
                              onClick={() => {
                                window.scrollTo(0, 0);
                                fetchLeads(Number(p));
                              }}
        ...
        <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
          <DialogContent className="max-w-xl p-0 overflow-hidden border-none shadow-2xl [&>button]:text-white [&>button]:opacity-100 [&>button]:hover:text-white/80 transition-all">
            ...
            {selectedLead && (
              <div className="p-6 space-y-6 bg-white">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-bold text-foreground flex items-center gap-1.5">
                      ...
                      {selectedLead.Cname || "Not provided"}
                    </p>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-bold text-foreground flex items-center gap-1.5">
                      ...
                      {selectedLead.mobile || "Not provided"}
                    </p>
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100">
                  ...
                  <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 text-sm leading-relaxed text-slate-700 whitespace-pre-wrap italic break-words max-h-[300px] overflow-y-auto shadow-inner">
