...
const ManageMagazines = () => {
  ...
  useEffect(() => { fetchMagazines(pagination.currentPage); }, []);
  ...
  const handleSave = async () => {
    ...
    if (!form.report_images) { toast.error("Cover image is required"); return; }
    if (!form.pdf) { toast.error("PDF is required"); return; }
                ...
                <div className="md:col-span-2 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        className="w-full h-10 px-3 rounded-md border border-input bg-background/50 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-all"
                        value={form.language}
                  ...
                  <div className="bg-muted/30 p-4 rounded-lg flex items-center justify-between border border-border/50">
                    <div className="space-y-0.5">
                      <label className="text-sm font-medium">Content Access</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <label className="flex flex-col items-center justify-center h-24 gap-1 cursor-pointer border-2 border-dashed border-border/60 rounded-xl p-4 hover:border-primary/50 hover:bg-primary/5 transition-all group">
                        ...
                        <span className="text-[11px] font-medium text-center truncate w-full px-2">
                          {uploadingPdf ? "Uploading..." : form.pdf ? "File path: " + form.pdf.split('/').pop() : "Click to upload PDF"}
                        </span>
              ...
              <div key={mag.id} className="bg-card border border-border rounded-xl overflow-hidden group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                <div className="relative aspect-[3/4.2] bg-muted overflow-hidden">
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1.5">
                    <Badge className={`${mag.language === 'hindi' ? "bg-orange-500/90" : "bg-blue-500/90"} backdrop-blur-sm border-none text-[10px] px-2 py-0.5`}>
                      {mag.language.toUpperCase()}
                    </Badge>
                    <Badge
                      variant={mag.pdf_lock == 1 || mag.pdf_lock === true ? "destructive" : "secondary"}
                      className="backdrop-blur-sm border-none text-[10px] px-2 py-0.5"
                    >
