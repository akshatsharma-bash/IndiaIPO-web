...
const DailyDigestCampaign = () => {
  const [searchParams] = useSearchParams();
  const [digests, setDigests] = useState<Digest[]>([]);
  const [tempDigestId, setTempDigestId] = useState<string | null>(searchParams.get("id"));
  ...
  useEffect(() => {
    ...
    if (searchParams.get("id")) {
        handleLoadCampaign(searchParams.get("id")!);
    }
  }, []);
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-6 rounded-[2rem] border border-border shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            >
              <SelectTrigger className="w-full md:w-[350px] h-12 bg-background border-border shadow-inner rounded-xl font-medium">
            ...
            <Button 
              className="h-12 px-8 rounded-xl font-bold bg-primary shadow-lg shadow-primary/20 hover:scale-[1.02] transition-transform active:scale-95"
              ...
              >
                ...
                <CardContent>
                  <div className="flex items-baseline justify-between">
                    <div className="text-4xl font-black">{stats?.total ?? listData.length}</div>
                ...
                <CardHeader className="bg-muted/30 border-b border-border/40 py-6 px-8">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <CardTitle className="text-xl capitalize font-black tracking-tight">
                    ...
                    <Badge variant="outline" className="bg-white/50 px-4 py-1.5 rounded-full font-bold border-border shadow-sm">{listData.length} Records</Badge>
                  </div>
                </CardHeader>
                    ...
                    <Table>
                      <TableHeader className="bg-muted/50 sticky top-0 z-10 backdrop-blur-md">
                        <TableRow className="border-border/40">
                          ...
                          {filter === 'failed' && <TableHead className="font-bold">Error Details</TableHead>}
                          {filter !== 'audience' && <TableHead className="font-bold">Timestamp</TableHead>}
                        ...
                        {fetchingList ? (
                          ...
                          <TableRow>
                            <TableCell colSpan={filter === 'failed' ? 5 : 4} className="h-48 text-center">
                              <div className="flex flex-col items-center justify-center gap-2 opacity-40">
                                ...
                                <span className="font-bold">No {filter} records found</span>
                              </div>
                            </TableCell>
                          </TableRow>
                        ) : (
                          listData.map((job, idx) => (
                            <TableRow key={job.id || job.user_email || idx} className="hover:bg-muted/30 transition-colors border-border/20">
                              ...
                              <TableCell className="font-semibold py-4">{job.user_email}</TableCell>
            ...
            <div className="space-y-6">
              ...
              <Card className="border-border/60 shadow-xl overflow-hidden rounded-[2rem]">
                <CardHeader className="bg-muted/30 border-b border-border/40 py-5">
                  <CardTitle className="text-lg font-bold tracking-tight">Digest Preview</CardTitle>
                </CardHeader>
                <CardContent className="p-6 space-y-5">
                  {selectedDigest?.image && (
                    <div className="rounded-2xl overflow-hidden border border-border shadow-md bg-slate-50 group relative">
                      <img 
                        src={selectedDigest.image.startsWith('http') ? selectedDigest.image : `/${selectedDigest.image}`} 
                        alt="Preview" 
                      ...
                      <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <Button variant="secondary" size="sm" className="rounded-full shadow-lg" asChild>
                            <a href={selectedDigest.pdf || "#"} target="_blank" rel="noreferrer">
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedDigest?.pdf && (
                      <Button variant="outline" className="w-full rounded-xl border-border hover:bg-muted font-bold h-10" asChild>
                        <a href={selectedDigest.pdf.startsWith('http') ? selectedDigest.pdf : `/${selectedDigest.pdf}`} target="_blank" rel="noreferrer">
              ...
              <Card className="border-border/60 shadow-2xl rounded-[2rem] bg-slate-900 text-white overflow-hidden relative">
                ...
                <CardContent className="p-8 space-y-6 relative z-10">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {startingCampaign ? (
                        <><Loader2 className="h-7 w-7 mr-3 animate-spin" /> QUEUING...</>
                      ) : (stats?.pending ?? 0) > 0 ? (
                        <><Clock className="h-7 w-7 mr-3 animate-pulse" /> BROADCASTING</>
                      ) : (
                        <><Send className="h-7 w-7 mr-3" /> START CAMPAIGN</>
                      )}
                    </Button>
                  ...
                  {(stats?.failed ?? 0) > 0 && (
                    ...
                    >
                      {retryingFailed ? (
                        ...
                        <><RefreshCcw className="h-4 w-4 mr-2" /> Retry {stats?.failed} Failures</>
                      )}
                    </Button>
                  )}
...
export default DailyDigestCampaign;
