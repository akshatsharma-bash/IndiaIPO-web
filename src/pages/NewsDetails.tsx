...
const quickInsights = [
  ...
  { icon: Bell, title: "Regulatory Alerts", desc: "Instant regulatory updates and circular analysis from market bodies." },
    ...
    fetch("/api/news")
      ...
      .then((data) => {
        ...
        if (Array.isArray(newsArray)) {
          setLatestNews(
            newsArray.filter((item: any) => item.slug !== slug && item.id.toString() !== slug).slice(0, 8)
          );
        }
      })
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {latestNews.slice(0, 3).map((item, i) => {
                      ...
                      return (
                        <Link key={item.id} to={`/news/detail/${item.slug || item.id}`}
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
                  ...
                  <div className="p-6 text-center">
                    ...
                    <p className="text-slate-500 text-sm leading-relaxed mb-5">
                      Want to list your company? Our expert advisors are ready to guide you through every step.
                    </p>
                    <Link to="/contact"
                        ...
                        return (
                          <Link key={item.id} to={`/news/detail/${item.slug || item.id}`}
                            ...
                            <div className="w-16 h-16 shrink-0 rounded-lg overflow-hidden bg-slate-100">
                              <img src={getImgSrc(item.image) || fallbackImage} alt=""
                                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                onError={(e: any) => (e.target.src = fallbackImage)} />
                            </div>
                            <div className="flex-1 min-w-0">
