...
const quickInsights = [
  {
    icon: TrendingUp,
    title: "IPO GMP Tracker",
    desc: "Live grey market premium updates for all open and upcoming IPOs.",
  },
  {
    icon: Bell,
    title: "Regulatory Alerts",
    desc: "Instant regulatory updates and circular analysis from market bodies.",
  },
  {
    icon: BarChart3,
    title: "Market Data",
    desc: "NSE/BSE index data, FII/DII flows, and sector analysis.",
  },
  {
    icon: Globe,
    title: "Economy Updates",
    desc: "Macro-economic policy changes affecting capital markets.",
  },
];
...
const ArticleContent = ({ content }: { content: string }) => {
  ...
  const activeContent = content.split(/End of Article/i)[0].trim();

  if (hasHtmlTags(activeContent)) {
    return (
      <>
        <style>{`
  ...
  const rawParagraphs = activeContent
    .split(/\n{2,}|\r\n\r\n/)
    .map((p) => p.trim())
    .filter(Boolean);
  ...
  paragraphs.forEach((para, i) => {
    if (/^(also read|see also|read also|related)/i.test(para)) {
      renderedParagraphs.push(
        <div
          key={`also-${i}`}
          className="my-5 p-4 rounded-xl border-l-4 flex items-center gap-3"
        ...
        >
          ...
          <span className="text-sm font-semibold text-slate-600">
            {para.replace(/^(also read|see also|read also|related)[:\s]*/i, "")}
          </span>
        </div>,
      );
      return;
    ...
    if (/^(note|disclaimer|important|please note)/i.test(para)) {
      renderedParagraphs.push(
        <div
          key={`note-${i}`}
          className="my-5 p-4 rounded-xl border border-slate-200 bg-slate-50 flex gap-3"
        >
          <span className="text-xs font-black uppercase tracking-widest text-slate-400 shrink-0 pt-0.5">
            Note
          </span>
          <p className="text-sm font-medium text-slate-500 leading-relaxed m-0">
            {para.replace(
              /^(note|disclaimer|important|please note)[:\s]*/i,
              "",
            )}
          </p>
        </div>,
    ...
    if (isHeading) {
      renderedParagraphs.push(
        <h2
          key={`h-${i}`}
          className="text-xl font-black mt-10 mb-5 px-4 py-3 border-l-8 rounded-r-xl"
          style={{
            color: "#001529",
            background: "#eff6ff",
    ...
    renderedParagraphs.push(
      <div key={`p-${i}`}>
        ...
        <p
          className={`leading-[1.9] text-slate-600 mb-5 font-medium ${isFirst ? "text-[1.05rem]" : "text-[0.96rem]"}`}
        >
          {isFirst && (
            <span
              className="float-left text-6xl font-black leading-none mr-3 mt-1"
  ...
  const factsBox =
    keyFactLines.length >= 2 ? (
      <div
        key="facts"
        className="my-8 rounded-2xl overflow-hidden border border-slate-200 shadow-sm"
      >
        <div
        ...
        <div className="divide-y divide-slate-100">
          {keyFactLines.map((f, fi) => (
            <div
              key={fi}
              className={`flex items-center justify-between px-5 py-3 ${fi % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"}`}
            >
              <span className="text-xs font-black uppercase tracking-widest text-slate-400">
                {f.label}
              </span>
              <span className="text-sm font-black" style={{ color: "#001529" }}>
                {f.value}
              </span>
            </div>
          ))}
...
export default function NewsDetails() {
        ...
        if (Array.isArray(newsArray)) {
          setLatestNews(
            newsArray
              .filter(
                (item: any) =>
                  item.slug !== slug && item.id.toString() !== slug,
              )
              .slice(0, 8),
          );
        }
  ...
  let seoTitle = `${news.title} | India IPO News`;
  if (seoTitle.length > 55) seoTitle = seoTitle.substring(0, 52) + "...";

  let seoDesc = news.description || "Latest news and updates from India IPO.";
  if (seoDesc.length > 140) seoDesc = seoDesc.substring(0, 137) + "...";
    ...
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      ...
      <main className="flex-1">
        <section className="relative h-[400px] md:h-[550px] w-full overflow-hidden">
          <img
            src={getImgSrc(news.image) || fallbackImage}
            alt={news.title}
          ...
          <div className="absolute inset-0 flex flex-col justify-end">
            <div className="container mx-auto px-4 pb-10 md:pb-14">
              <div className="flex items-center gap-2 text-white/50 text-xs mb-5 flex-wrap">
                ...
                <ChevronRight className="h-3 w-3" />
                <span className="text-white/70 line-clamp-1 max-w-[150px]">
                  {news.title}
                </span>
              </div>
            ...
            <div className="xl:col-span-2">
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-200 flex-wrap">
                ...
                <a
                  ...
                  className="w-10 h-10 rounded-xl bg-[#1877F2]/10 text-[#1877F2] flex items-center justify-center hover:bg-[#1877F2] hover:text-white transition-all hover:scale-110 shadow-sm transition-all duration-300"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-slate-100 text-slate-800 flex items-center justify-center hover:bg-slate-800 hover:text-white transition-all hover:scale-110 shadow-sm transition-all duration-300"
                >
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-[#0A66C2]/10 text-[#0A66C2] flex items-center justify-center hover:bg-[#0A66C2] hover:text-white transition-all hover:scale-110 shadow-sm transition-all duration-300"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-green-50 text-green-500 flex items-center justify-center hover:bg-green-500 hover:text-white transition-all hover:scale-110 shadow-sm transition-all duration-300"
                >
                ...
                <button
                  ...
                  style={{
                    background: copied
                      ? "linear-gradient(135deg, #001529, #003380)"
                      : "#f1f5f9",
                    color: copied ? "white" : "#001529",
                  }}
                >
              ...
              <div className="article-content space-y-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-7 md:p-10 mb-8">
              ...
              {latestNews.slice(0, 3).length > 0 && (
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h3 className="text-xl font-black text-[#001529]">
                      You May Also Like
                    </h3>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-slate-100">
                            <img
                              src={getImgSrc(item.image) || fallbackImage}
                              alt={item.title}
                          ...
                          <div className="flex-1 min-w-0">
                            {item.category && (
                              <span className="text-[9px] font-black uppercase tracking-widest text-[#f59e08]">
                                {item.category}
                              </span>
                            )}
              ...
              <div className="sticky top-24 space-y-5">
                <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
                  <div
                    className="h-1.5"
                    style={{
                      background: "linear-gradient(90deg, #001529, #f59e08)",
                    }}
                  />
                  <div className="p-6 text-center">
                    ...
                    <h3 className="text-xl font-black text-[#001529] mb-2">
                      IPO Advisory
                    </h3>
                    <p className="text-slate-500 text-sm leading-relaxed mb-5">
                      Want to list your company? Our expert advisors are ready
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: "2px solid #f59e08",
                    }}
                  >
                    <Flame className="h-4 w-4 text-[#f59e08]" />
                    <h3 className="font-black text-[#001529] text-sm uppercase tracking-widest">
                      Latest News
                    </h3>
                  </div>
                  <div className="p-4 space-y-3">
                ...
                <div className="bg-[#001529] rounded-2xl p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <Mail className="h-4 w-4 text-[#f59e08]" />{" "}
                      info@indiaipo.in
                    </a>
                    <a
                      href="tel:+917428337280"
        ...
        <section className="bg-gradient-to-r from-[#001529] via-[#002147] to-[#003380] py-20 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            ...
            <h2 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight">
              Never Miss an <span className="text-[#f59e08]">IPO Alert</span>
              <br />
              or Market Update
            </h2>
            <p className="text-white/60 max-w-xl mx-auto text-base font-medium mb-10 leading-relaxed">
              Get personalized IPO alerts, GMP updates, and market news on
              WhatsApp. Free to subscribe — unsubscribe anytime.
            </p>
