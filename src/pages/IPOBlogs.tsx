          ...
          {loading ? (
            ...
            <div className="text-center py-24 border-2 border-dashed border-slate-200 rounded-[2.5rem] bg-white shadow-sm">
              ...
              {search.trim() && (
                <button
                  onClick={() => setSearch("")}
                  className="px-8 py-3 rounded-xl bg-slate-100 text-slate-600 font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all"
                >
                  Clear Search
                </button>
              )}
                      ...
                      <div className="h-full bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1.5 transition-all flex flex-col">
                        ...
                        <div className="aspect-video bg-white relative overflow-hidden flex items-center justify-center">
                          ...
                          <div className="absolute top-3 right-3">
                            {/* {String(blog.upcoming) === "1" ? (
                              <span className="px-2.5 py-1 rounded-full text-[10px] font-black"
                                style={{ background: "rgba(245,158,8,0.15)", color: "#d97706", border: "1px solid rgba(245,158,8,0.3)" }}>
                                Upcoming
                              </span>
                            ) : (
                              <span className="px-2.5 py-1 rounded-full text-[10px] font-black"
              ...
              <div className="mt-14 flex flex-col items-center gap-6">
                <div className="flex items-center justify-center gap-1.5 sm:gap-2">
                  <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                    className="flex items-center justify-center gap-1.5 w-9 h-9 sm:w-auto sm:px-5 sm:h-11 rounded-xl font-black text-[10px] sm:text-xs text-[#001529] bg-white border border-slate-200 transition-all disabled:opacity-40 shadow-sm"
                  >
                    ...
                    {range.map((p, i) => (
                      p === "..." ? (
                        ...
                        <button key={p} onClick={() => setPage(p as number)}
                          className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl font-black text-xs sm:text-sm transition-all shadow-sm ${p === page
                  ...
                  <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                    className="flex items-center justify-center gap-1.5 w-9 h-9 sm:w-auto sm:px-5 sm:h-11 rounded-xl font-black text-[10px] sm:text-xs text-white transition-all disabled:opacity-40 shadow-md"
