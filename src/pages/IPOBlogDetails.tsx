...
const IPOBlogDetails = () => {
    ...
    const fetchBlog = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (pathname.startsWith("/ipo-blogs/")) {
            if (category === "ipo_blogs") {
              ...
              navigate("/ipo-blogs", { replace: true });
              return;
            }
          } else if (pathname.startsWith("/daily-reporter/")) {
            if (category === "ipo_blogs") {
              navigate(`/blogs/${slug}`, { replace: true });
              return;
            } else if (category === "ipo_updates") {
              ...
              navigate("/daily-ipo-digest", { replace: true });
              return;
            }
          }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0f5ff]">
      <SEOHead
        title={
          stripHtml(blog.meta_title) || `${stripHtml(blog.title)} - India IPO`
        }
        description={
          stripHtml(blog.description) ||
          `Read details and updates about ${stripHtml(blog.title)}`
        }
        keywords={blog.keyword || undefined}
        ogImage={getImageUrl(blog.image) || undefined}
              ...
              {blog.category === "daily_reporter" ? (
                <div className="flex flex-wrap gap-3">
                  {blog.linked_digest_id && (
                    <>
                      <Link
                        to={`/daily-ipo-digest/view/${blog.linked_digest_id}`}
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 mt-8">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 text-center">
                    ...
                    >
                      <Link to="/services">View All Advisory Services</Link>
                    </Button>
                  </div>
                </div>
              </div>
