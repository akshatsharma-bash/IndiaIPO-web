...
const WeeklyReporterViewer = () => {
  ...
  useEffect(() => {
    const fetchDigest = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Weekly report not found");
        const data = await res.json();
        setDigest(data);
      } catch (err) {
        console.error(err);
        toast.error("Failed to load weekly report");
        navigate("/weekly-ipo-report");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground font-medium">Preparing weekly report...</p>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen bg-[#F8FAFC] flex flex-col ${isFullScreen ? 'overflow-hidden fixed inset-0 z-[100]' : ''}`}>
      <SEOHead 
        title={`${digest.title} | Weekly IPO Reporter`}
        description={`Read the weekly report: ${digest.title}. Complete weekly IPO digest and market insights.`}
      />
...
export default WeeklyReporterViewer;
